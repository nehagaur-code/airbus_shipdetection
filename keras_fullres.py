...
masks = pd.read_csv('train_ships.csv')
...
print(unique_img_ids.sample(7))



from sklearn.model_selection import train_test_split
train_ids, validation_ids = train_test_split(unique_img_ids, test_size=0.33,
...
def sample_ships(in_df, base_rep=1500):
    if in_df['ships'].values[0] == 0:
        return in_df.sample(base_rep//3,replace=True)
    else:
...
fig.savefig('overview_keras_model.png')
...
segmentation_model.compile(optimizer=Adam(1e-4, decay=1e-6), loss=dice_p_bce, metrics=[dice_coef, 'binary_accuracy', true_positive_rate])



# add mode checkpointing

from keras.callbacks import ModelCheckpoint, LearningRateScheduler, EarlyStopping, ReduceLROnPlateau

weight_path = "{}_weights.best.hdf5".format(segmentation_model)
...
reduceLROnPlateau = ReduceLROnPlateau(monitor='val_dice_coef', factor=0.5, patience=3,
...
callbacks=[checkpoint, early, reduceLROnPlateau]
step_count = 3
# step_count = 3
NB_EPOCHS = 1
aug_gen = create_aug_gen(make_image_gen(balanced_train_df))
loss_history = [segmentation_model.fit_generator(aug_gen, 
...
def show_loss(loss_history):
    ...
    _ = ax1.plot(epoch, np.concatenate([mh.history['loss'] for mh in loss_history]), 'b-',
                 epoch, np.concatenate([mh.history['val_loss'] for mh in loss_history]), 'r-')
    ax1.set_title('Loss')
    ...
    _ = ax4.plot(epoch, np.concatenate(
...
pred_y = segmentation_model.predict(valid_x)
...
if IMG_SCALING is not None:
    ...
    fullres_model.add(segmentation_model)
    fullres_model.add(layers.UpSampling2D(IMG_SCALING))
else:
    fullres_model = seg_model
fullres_model.save('fullres_model.h5')
from keras.models import load_model
model = load_model('fullres_model.h5')
...
fig.savefig('test_predictions_new.png')
...
submission_df.to_csv('submission_autodetection.csv', index=False)
print(submission_df.sample(3))
